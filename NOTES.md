...
- **Configurable sandbox network (Codex).** GitHub runner networking still did not expose the host alias to the sandbox reliably, so `NICO_DOCKER_NETWORK` can now override the sandbox network. The Action smoke workflow runs both Juice Shop and Nico's sandbox with host networking so `localhost:3000` is shared on the runner. Default remains `bridge`.
