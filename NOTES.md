...
- **Action smoke PR (Codex).** Draft PR branch `codex/verify-action-dashboard-polish` adds `.github/workflows/nico-action-smoke.yml`, which starts OWASP Juice Shop, runs the reusable Action from the PR checkout, posts the sticky PR comment, uploads the report artifact, and sets `fail-on-severity: critical` so the severity gate is exercised against a real confirmed finding. The first run caught an `action.yml` metadata bug on master: GitHub rejects `${{ ... }}` expressions inside input descriptions before any scan step starts.
